...
and is is provided by Statistisches Bundesamt ("Gemeindeverzeichnis", 30.09.2021 available at 
[destatis](https://www.destatis.de/DE/Themen/Laender-Regionen/Regionales/Gemeindeverzeichnis/_inhalt.html), 
see "Administrative breakdown" -> "30.09.2021 quarterly issue"). From S3, the file `AuszugGV3QAktuell.xlsx` 
...
A combination of the table_rental_location (is a part of the first dataset) and table_stations 
...
After completing each etl step, the resulting tables will be persisted in S3. 
...
- create a S3 bucket and set the parameters `OUTPUT_PATH=<your AWS S3-URI>` in dl-cfg file,
- create an AWS EMR instance.

- Afterwards, we set up the Photon (by collecting photon jar and photon-db), and test Photon geocoding service.

The file dl.cfg collects all relevant information for the ELT pipeline, what we are about to configure. 
The file dl_example.cfg is a template. Therein we have to set:  
...
2. Create S3 bucket. Choose an appropriate region (like Irland, if you are located in Europe).
...
[S3]
...
INPUT_PATH=<your AWS S3-URI>
OUTPUT_PATH=<your AWS S3-URI>
```

(INPUT and OUTPUT must not share the equal address. Provided the access privilege, you could set the OUTPUT_PATH 
...
4. Get raw data from my S3 Bucket:  
   ...
   - Duplicate the data structure in your S3 bucket using CloudShell:   
      `aws s3 cp s3://f3de543c84 <your AWS S3-URI> --recursive`
<!--
...
As a result, 
you will get the subdirectories */data_collection/* and */data_mart/* with all required files 
...
Photon (https://photon.komoot.io/) as local geocoding service requires the following prerequisites:
   ...
   - Create and change into new subdirectory `./photon`    
   ...
   `curl --output photon-0.3.5.jar https://github.com/komoot/photon/releases/download/0.3.5/photon-0.3.5.jar`    
<!--
...
-->  
   ...
   `curl --insecure --output photon-db-latest.tar.bz2 https://download1.graphhopper.com/public/extracts/by-country-code/de/photon-db-de-latest.tar.bz2  | pbzip2 -cd | tar x`  
